...
def weather_to_db(text):
    ...
    vals2 = [weathers['weather'][0]['id'],weathers['weather'][0]['main'],weathers['weather'][0]['description'],
            ...
            weathers['wind']['deg'],weathers['sys']['sunrise'],weathers['sys']['sunset'],datetime.today().replace(microsecond=0)]
    
    vals2[8] = datetime.fromtimestamp(vals2[8])
    
    vals2[9] = datetime.fromtimestamp(vals2[9])
    engine.execute("insert into weather values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", vals2)
